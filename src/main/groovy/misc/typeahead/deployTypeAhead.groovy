...
public void deployCollections(FusionClient fusionClient, String appName, ConfigObject config) {
    ...
    if (existingCollections) {
        config.collections.each { String label, Map collectionConfig ->
            ...
            if (taCollection) {
                log.warn "Collection id($destId) already exists, not recreating : ${taCollection}"
            } else {
                log.info "Create new collection ($destId) in app: $appName -- solrParams: $collectionConfig"
                FusionResponseWrapper frwBCollection = fusionClient.createCollection(destId, collectionConfig, appName, false)
